...
window.onload = function () {
    ...
    for(var x = 0; x < MAP_WIDTH; x++) {
        ...
        for(var y = 0; y < MAP_HEIGHT; y++) {
            tiles[x].push({ground: (Math.random() > 0.15 ? GROUND_TYPES.grass : GROUND_TYPES.stone), building: {type: (Math.random() > 0.50 ? BUILDING_TYPES.empty : BUILDING_TYPES.forest), data: {}}});
        }
    }
